...
class ApiLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if path.startswith("/mcp"):
            # Separate protocol surface (one POST per MCP tool call from ai-app) — would
            # dominate this human-facing REST log; that traffic is already implicitly
            # visible via the Langfuse traces it triggers.
