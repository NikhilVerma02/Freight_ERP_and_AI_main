  ...
  return (
    <div className="relative" onMouseEnter={show} onMouseLeave={hide}>
      <button
        ...
        className="flex h-8 w-8 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-navy-700 dark:hover:text-slate-200"
      >
        <ActiveIcon theme={theme} />
      </button>

      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-40 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl dark:border-navy-600 dark:bg-navy-800">
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 px-3.5 py-2.5 text-sm font-medium transition-colors ${
                  active
                    ? "text-accent dark:text-accent-light bg-accent/8 dark:bg-accent/15"
                    : "text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-navy-700 dark:hover:text-white"
                }`}
              >
